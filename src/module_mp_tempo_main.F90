module module_mp_tempo_main
  ...
  use module_mp_tempo_params, only : wp, sp, dp, &
    min_qv, roverrv, rdry, r1, r2, nt_c_max, nt_c_min, t0, nrhg, rho_g, cloud_fraction_rh, cloud_fraction_rh_pbl_water, &
    meters3_to_liters, eps, aero_max, nwfa_default, nifa_default, eps, cf_low, xm0i, lsub, cloud_fraction_rhi
  ...
  type :: ty_tend
    real(dp), pointer, contiguous, dimension(:) :: &
      ...
      prw_sgi, prw_sbl, prw_slw, prw_sge, pnc_sgs, & ! cloud fraction
      pra_sgi, pra_sbl, pra_slw, pra_sge, pra_ini, prw_ini ! cloud fraction
  end type
  ...
  subroutine tempo_main(tempo_cfgs, &
    ...
    real(wp), dimension(:), intent(in), optional :: hpbl1d !! boundary layer height \([m]\)
    real(wp), dimension(:), intent(in), optional :: xland1d !! land values = 1 for land, 2 for ocean
    ...
    ! local variables
    real(wp) :: tempc, tc0, odt, hgt, hgt_cf, sgs_nc, uval
    real(wp), dimension(kts:kte) :: cf_rh, cf_rhi
    ...
    tend%prw_sgi => tend_work(:, 75)
    tend%pra_sgi => tend_work(:, 76)
    tend%prw_sbl => tend_work(:, 77)
    tend%pra_sbl => tend_work(:, 78)
    tend%prw_slw => tend_work(:, 79)
    tend%pra_slw => tend_work(:, 80)
    tend%prw_sge => tend_work(:, 81)
    tend%pra_sge => tend_work(:, 82)
    tend%pnc_sgs => tend_work(:, 83)
    tend%pra_ini => tend_work(:, 84)
    tend%prw_ini => tend_work(:, 85)
...
!    hgt_cf = max(hpbl1d(1), 500._wp)
    ...
    do k = 1, nz
      ...
      cf_rhi(k) = cloud_fraction_rhi
      hgt = hgt + dz1d(k)
      
      if (dz1d(k) > 100._wp .and. temp(k) > (t0 - 68._wp)) then
         cf_rhi(k) = cloud_fraction_rhi - dz1d(k)*0.0001_wp
      endif

      if (hgt < hgt_cf) then
         cf_rh(k) = cloud_fraction_rh_pbl_water
      else
         uval = 1.5 - 0.0025*(pres(k)/100.)         
         if (xland1d(1) > 1.5) then !ocean
            cf_rh(k) = cloud_fraction_rh_pbl_water
  !!          cf_rh(k) = max(min(0.95 + ((0.078*uval) / (1.-uval*uval)), 0.99), 0.87)      
         else ! land
            cf_rh(k) = max(min(0.95 + ((0.078*uval) / (1.-uval*uval)), 0.99), 0.87)                  
!!            cf_rh(k) = max(min(0.918485 + ((0.047273*uval) / (1.-uval*uval)), 0.99), 0.87)      
         endif
      endif
      
!      if ((xland1d(1) > 1.5) .and. (hgt <= hgt_cf + 0.)) then ! ocean
!        cf_rh(k) = cloud_fraction_rh_pbl_water
!      endif
     
!      if (xland1d(1) < 1.5) then ! land
!         if (hgt < hpbl1d(1)) then 
!            cf_rh(k) = cloud_fraction_rh_pbl_water
!         else
!            uval = 1.5 - 0.0025*(pres(k)/100.)
!            cf_rh = max(min(0.918485 + ((0.047273*uval) / (1.-uval*uval)), 0.95), 0.87)
!            if (pres(k) <= 85000.) then
!               cf_rh(k) = min(0.95, (375500.-pres(k)) / 350000.)
!            endif
!         endif
!      endif
    ...
    call thermo_vars(qv, temp, pres, rho, rhof, rhof2, qvs, delqvs, qvsi, &
      ...
      supersaturated, cf_rh, above_cloud_fraction_rh)
    ...
    if (.not. tempo_cfgs%turn_off_micro_flag) then
      call warm_rain(rhof, l_qc, rc, nc, ilamc, mvd_c, l_qr, rr, nr, mvd_r, tend, qca1d, odt)
    endif 
    if (.not. tempo_cfgs%turn_off_micro_flag) then
    ...
    if (.not. tempo_cfgs%turn_off_micro_flag) then
      call ice_processes(rhof, rhof2, rho, w1d, temp, cf_rhi, qv, qvsi, tcond, diffu, &
      vsc2, ssati, l_qi, ri, ni, ilami, l_qs, rs, smoe, smof, smo1, rr, nr, &
      ilamr, mvd_r, l_qg, rg, ng, ilamg, idx_bg, qia1d, tend, odt)
    endif
    if (.not. tempo_cfgs%turn_off_micro_flag) then
    ...
    if (.not. tempo_cfgs%turn_off_micro_flag .and. tempo_cfgs%cloud_condensation_flag) then
      ...
      do k = 1, nz
!!!         if ((satw(k) <= cf_rh(k) .and. qcten_bl1d(k) < eps) .or. satw(k) >= 1.) then
         if (satw(k) <= cf_rh(k) .or. satw(k) >= 1.) then            
          ...
          if (rc(k) > r1) then
             qcfracten(k) = qcfracten(k) + ((1.+tend%prw_vcd(k)*dt/rc(k))**0.5 - 1.) * odt * qca1d(k)
          else
             if (tend%prw_vcd(k)*qca1d(k) > r1) then
                qcfracten(k) = qcfracten(k) + 0.25 / odt
             endif
          endif
           
!          if (tend%prw_vcd(k)*qca1d(k) > r1) then
!            if (qca1d(k) > cf_low) then
!               qcfracten(k) = qcfracten(k) + &
!                    ((1.+tend%prw_vcd(k)/qc1d(k)/rho(k))**0.5 - 1.) * qca1d(k)                             
!!! AAJ TEST 4                max((5.57_wp*(1000._wp*tend%pnc_wcd(k)/qca1d(k)*dt)**(0.78_wp) * odt), cf_low)
!           else
!              qcfracten(k) = qcfracten(k) + &
!                   ((1.+tend%prw_vcd(k)/qc1d(k)/rho(k))**0.5 - 1.) * 0.1
!!! AAJ TEST 4                max((5.57_wp*(1000._wp*tend%pnc_wcd(k)*dt)**(0.78_wp) * odt), cf_low)                            
!!              qcfracten(k) = 1._wp
!           endif
...
!                  ((1.-tend%prw_vcd(k)/qc1d(k)/rho(k))**0.5 - 1.) * qca1d(k)                                                                  
!!! AAJ TEST 4                max((5.57_wp*(1000._wp*tend%pnc_wcd(k)/qca1d(k)*dt)**(0.78_wp) * odt), cf_low)
!!!             max((5.57_wp*(1000._wp*tend%pnc_wcd(k)/qca1d(k))**(0.78_wp) * odt), cf_low)           
!          endif          
      ...
      do k = 1, nz
        ...
        rho(k) = roverrv*pres(k)/(rdry*temp(k)*(qv(k)+roverrv))
      enddo 
      ! don't update thermo yet
    endif 

   ! cloud fraction
    call ice_cloud_fraction(dz1d, temp, cf_rhi, l_qi, rho, qv, qvsi, qi1d, l_qs, qs1d, ssati, qia1d, &
      qca1d, qiten_bl1d, qiten, w1d, ocp, tend, dt, odt)

    call liquid_cloud_fraction(cf_rh=cf_rh, temp=temp, pres=pres, dz1d=dz1d, l_qc=l_qc, l_qr=l_qr, rho=rho, qv=qv, &
      ...
      qvten_bl1d=qvten_bl1d, qcten_bl1d=qcten_bl1d, thten_bl1d=thten_bl1d, nwfa=nwfa, qc_bl1d=qc_bl1d, qcfrac_bl1d=qcfrac_bl1d, tend=tend, dt=dt, odt=odt)

    do k = 1, nz
      if (satw(k) > cf_rh(k) .and. satw(k) < 1.) then
        qvten(k) = qvten(k) - tend%prw_sgi(k) - tend%prw_slw(k) - tend%prw_sbl(k) - tend%prw_sge(k) ! - tend%prw_ini(k)
        qcten(k) = qcten(k) + tend%prw_sgi(k) + tend%prw_slw(k) + tend%prw_sbl(k) + tend%prw_sge(k)
        ncten(k) = ncten(k) + tend%pnc_sgs(k)
        nwfaten(k) = nwfaten(k) - tend%pnc_sgs(k)
        tten(k) = tten(k) + ((lvap(k)*ocp(k) * &
          (tend%prw_sge(k) + tend%prw_sgi(k) + tend%prw_slw(k) + tend%prw_sbl(k)))* (1._wp / rho(k)))
        qcfracten(k) = qcfracten(k) + tend%pra_sgi(k) + tend%pra_slw(k) + tend%pra_sbl(k) + tend%pra_sge(k)
...
! AAJ        qca1d(k) = qca1d(k) + qcfracten(k)*dt
        ...
        !qiten(k) = qiten(k) + tend%prw_ini(k)
        !niten(k) = niten(k) + tend%prw_ini(k)/xm0i 
! AAJ        qia1d(k) = qia1d(k) + qifracten(k)*dt
     endif
      tten(k) = tten(k) + ((lsub*ocp(k)*(tend%prw_ini(k)))* (1._wp / rho(k)))

      qifracten(k) = qifracten(k) + tend%pra_ini(k)
        
      qvten(k) = qvten(k) - tend%prw_ini(k)
      qiten(k) = qiten(k) + tend%prw_ini(k)
      niten(k) = niten(k) + (tend%prw_ini(k))/(0.1*xm0i)

!      if (tend%prs_ide(k) < 0. .and. qs1d(k) > r1 .and. temp(k) < 263.15) then
!         qifracten(k) = qifracten(k) + max((5.57_wp*(-1000._wp*tend%prs_ide(k)*dt)**(0.78_wp) * odt), cf_low)
!         qiten(k) = qiten(k) + 
!         niten(k) = niten(k) +
!         qsten(k) = qsten(k) + 
!      endif
      ...
      if(ri(k) > r1) then
         !AAJ TEST 4         qifracten(k) = qifracten(k) + ((1.-tend%prs_iau(k)*dt/qi1d(k)/rho(k))**0.5 - 1.) * qia1d(k)
         qifracten(k) = qifracten(k) + ((1.-tend%prs_iau(k)*dt/ri(k))**0.5 - 1.) * odt * qia1d(k)
         qifracten(k) = qifracten(k) + ((1.-(tend%prs_sci(k)+tend%prs_ide(k)+tend%pri_ide(k))*dt/ri(k))**0.5 - 1.) * odt * qia1d(k)                  
!!!         qifracten(k) = qifracten(k) + ((1.-tend%prs_iau(k)/qi1d(k)/rho(k))**0.5 - 1.) * qia1d(k)         
         !! OFF 13 JUNE 2026  if (tend%pri_ide(k) < 0.) qifracten(k) = qifracten(k) + ((1.+tend%pri_ide(k)*dt/qi1d(k)/rho(k))**0.5 - 1.) * qia1d(k)         
      endif
      
      if(rc(k) > r1) then
         !! AAJ TEST4         qcfracten(k) = qcfracten(k) + ((1.-tend%prr_wau(k)*dt/qc1d(k)/rho(k))**0.5 - 1.) * qca1d(k)
         qcfracten(k) = qcfracten(k) + ((1.-tend%prr_wau(k)*dt/rc(k))**0.5 - 1.) * odt * qca1d(k)         
   !!!      qcfracten(k) = qcfracten(k) + ((1.-tend%prr_wau(k)/qc1d(k)/rho(k))**0.5 - 1.) * qca1d(k)         
      endif      
      
!!!!      nifaten(k) = nifaten(k) - tend%prw_ini(k)/xm0i
      
      if (qcfrac_bl1d(k) > 0.05 .and. qcfrac_bl1d(k) < 0.5 .and. temp(k) > 253.15 .and. &
        qcten_bl1d(k) > r1 .and. (ri(k)/rho(k)+rc(k)/rho(k)+rr(k)/rho(k)+qcten(k)*dt+qiten(k)*dt+qrten(k)*dt) <= r1) then
         qcfracten(k) = qcfracten(k) + max(qcfrac_bl1d(k) * odt, 0.075*odt)
         qcten(k) = qcten(k) + (qcfrac_bl1d(k)**(1./0.78))/5.57/1000.*odt
         qvten(k) = qvten(k) - (qcfrac_bl1d(k)**(1./0.78))/5.57/1000.*odt         

         sgs_nc = max(max(qcfrac_bl1d(k), 0.1)*nt_c_min, activate_cloud_number(temp(k), max(0.1_wp, w1d(k)), nwfa(k)))
         if (present(nc1d)) then
            ncten(k) = ncten(k) + 0.5_wp*(sgs_nc/rho(k)-nc1d(k) + abs(sgs_nc/rho(k)-nc1d(k)))*odt
         endif
           
         tten(k) = tten(k) + (lvap(k)*ocp(k) * &
              ( (qcfrac_bl1d(k)**(1./0.78))/5.57/1000.*odt ))* (1._wp / rho(k))         


         !         qcfrac = 5.57_wp*(1000._wp*qc)**(0.78_wp)
!         tend%prw_sgi(k) = qc_bl1d(k) * odt * rho(k)

!         if (tend%prw_sgi(k)*dt > r1) then
!            xnc = max(tend%pra_sgi(k)*dt*nt_c_min, activate_cloud_number(temp(k), max(0.1_wp, w1d(k)), nwfa(k)))
!            if (present(nc1d)) then
!               tend%pnc_sgs(k) = 0.5_wp*(xnc/rho(k)-nc1d(k) + abs(xnc/rho(k)-nc1d(k)))*odt
!            endif
!         endif
      endif
    ...
    if (.not. tempo_cfgs%turn_off_micro_flag) then
      call rain_evaporation(rho, temp, ssatw, lvap, tcond, diffu, vsc2, rhof2, &
        qv, qvs, l_qr, rr, nr, ilamr, cf_rh, l_qc, qca1d, tend, odt)
    ...
    substeps_sedi = 1
    if (any(l_qi)) then
      call ice_fallspeed(rhof, l_qi, ri, ilami, dz1d, vtri, vtni, qia1d, &
        substeps_sedi, ktop_sedi, dt)
      call sedimentation(xr=ri, vt=vtri, dz1d=dz1d, rho=rho, qifrac1d=qia1d, xten=qiten, limit=r1, &
        steps=substeps_sedi, ktop_sedi=ktop_sedi, precip=tempo_main_diags%ice_liquid_equiv_precip, dt=dt)
      call sedimentation(xr=ni, vt=vtni, dz1d=dz1d, rho=rho, xten=niten, qifrac1d=qia1d, limit=r2, &
        steps=substeps_sedi, ktop_sedi=ktop_sedi, dt=dt)
    ...
    do k = 1, nz
      ...
      !! AAJ ON 13 JUNE 2026 TEST 05
!! 15 JUNE TESET      if ((qs1d(k) > 1.e-4) .and. (qi1d(k) > 1.e-9)) then

      ! 01 JULY 2026
      if ((qi1d(k) + qs1d(k) > 1.e-4) .and. (qi1d(k) > 1.e-9) .and. qia1d(k) < 0.9) then
         qia1d(k) = min(qia1d(k) + max(5.57_wp*(1000._wp*qs1d(k))**(0.78_wp), cf_low), 0.9_wp)
      endif
   enddo
...
!      endif
  ...
  subroutine cloud_check_and_update(rho, l_qc, qc1d, nc1d, ncsave, rc, nc, qcfrac1d, &
    ...
    real(wp), dimension(:), intent(inout), optional :: nc1d, ncsave, qcfrac1d
    ...
    do k = 1, nz
      ...
      if (qc1d(k)+qcten(k)*dt > r1) then
        ...
        if (present(nc1d)) then
          ...
          if (hit_limit) ncten(k) = (nc(k)*qcfrac1d(k)/rho(k) - nc1d(k)) * odt
 ...
 subroutine ice_check_and_update(rho, l_qi, qifrac1d, qi1d, ni1d, ri, ni, &
    ...
    do k = 1, nz 
      ...
      if (qi1d(k)+qiten(k)*dt > r1) then
        ...
        if (present(qifrac1d)) then
          call cloud_fraction_check(qifrac1d(k), ri(k)/rho(k))
        ...
        if (hit_limit) niten(k) = (qifrac1d(k)*ni(k)/rho(k) - ni1d(k))*odt
        ni1d(k) = max(r2/rho(k), &
  ...
  subroutine thermo_vars(qv, temp, pres, rho, rhof, rhof2, qvs, delqvs, qvsi, &
    ...
    real(wp), dimension(:), intent(in) :: qv, temp, pres, rho, cf_rh
    ...
    do k = 1, nz
      ...
      if (satw(k) > cf_rh(k)) above_cloud_fraction_rh = .true.
    ...
    do k = 1, nz
      ...
      if (temp(k) < t0) then
        tten(k) = tten(k) + &
          ...
          tend%prs_scw(k) + tend%prg_scw(k) + tend%prg_gcw(k) - tend%prr_rcs(k) + &
          tend%prr_rci(k) - tend%prr_rcg(k)))*orho
      else
        tten(k) = tten(k) + &
  ...
  subroutine sedimentation(xr, vt, dz1d, rho, xten, limit, steps, ktop_sedi, qifrac1d, precip, dt)
    ...
    do k = nz, 1, -1
      sed_r(k) = vt(k)*xr(k)
      if (present(qifrac1d)) sed_r(k) = vt(k)*xr(k) * qifrac1d(k)
    enddo
    k = nz
    odz = 1._wp/dz1d(k)
    orho = 1._wp/rho(k)
...
!          afall = a_coeff*((4._wp*dens_g*9.8_wp)/(3._wp*rho(k)))**b_coeff
          ...
          IF ( .false. ) THEN
             IF ( .true. ) THEN
                afall = a_coeff*((4._wp*dens_g*9.8)/(3._wp*rho(k)))**b_coeff
             ELSE
                afall = a_coeff*((4._wp*dens_g*9.8)/(3._wp*rho_not))**b_coeff
             ENDIF
             afall = afall * visco(k)**(1._wp-2._wp*b_coeff)
          ELSE
             afall = (4._wp*dens_g*9.8/(3._wp*0.504843467198652_wp*rho_not))**b_coeff
          ENDIF          
  ...
  subroutine ice_fallspeed(rhof, l_qi, ri, ilami, dz1d, vt, vtn, qia1d, &
    substeps_sedi, ktop_sedi, dt)
    ...
    real(wp), intent(in) :: dt    
    real(wp), dimension(:), intent(in) :: rhof, dz1d, ri, qia1d
...
!!        vtn(k) = rhof(k)*av_i*cig(6) * ilami(k)**bv_i        
  ...
  subroutine rain_evaporation(rho, temp, ssatw, lvap, tcond, diffu, &
    vsc2, rhof2, qv, qvs, l_qr, rr, nr, ilamr, cf_rh, l_qc, qca1d, tend, odt)
    ...
    real(wp), dimension(:), intent(in) :: rho, temp, ssatw, lvap, tcond, diffu, vsc2, rhof2, &
      qv, qvs, rr, nr, cf_rh, qca1d
    logical, dimension(:), intent(in) :: l_qr, l_qc
    real(dp), dimension(:), intent(in) :: ilamr
    ...
    do k = 1, nz
      ...
      if (l_qc(k)) then
         if (ssatw(k) > cf_rh(k)-0.99) then
            tend%prv_rev(k) = tend%prv_rev(k) * (1.-qca1d(k))
            tend%pnr_rev(k) = tend%pnr_rev(k) * (1.-qca1d(k))
         endif
      endif
    enddo 
  ...
  subroutine ice_cloud_fraction(dz1d, temp, cf_rhi, l_qi, rho, qv, qvsi, qi1d, l_qs, qs1d, ssati, qifrac1d, &
      qcfrac1d, qiten_bl1d, qiten, w1d, ocp, tend, dt, odt)
    use module_mp_tempo_params, only : rv, cloud_fraction_rhi, eps, lsub, hgfrz

    real(wp), intent(in) :: dt, odt
    real(wp), dimension(:), intent(in) :: dz1d, temp, rho, qv, qvsi, ssati, qi1d, qs1d, qifrac1d, &
      qcfrac1d, w1d, ocp, qiten_bl1d, qiten, cf_rhi
    logical, dimension(:), intent(in) :: l_qi, l_qs
    type(ty_tend), intent(inout) :: tend
    real(wp) :: orho, omega, dqsdti, al, bs, sd, qi_mean, qtot_mean, ls_cond, &
      term1, term2, gterm, qifrac_, qi_, ssati_total, w_tmp
    integer :: k, nz
    ...
    do k = 1, nz
      if (qi1d(k) <= r1) then
        ...
        if (tend%pri_rfz(k)*dt > r1) then
          tend%pra_ini(k) = 1._dp*odt
        elseif (tend%pri_wfz(k)*dt > r1 .and. qcfrac1d(k) > cf_low) then
          tend%pra_ini(k) = max(qcfrac1d(k), cf_low)*odt
        else
          if (qiten(k)*dt > r1) tend%pra_ini(k) = min(max(ssati(k)+1._wp-cf_rhi(k), 0.1_wp), 1._wp) ! 0.25*odt 
       endif
    endif

    ! vertical motion (existing ice (which can submliate and destabilize) or large grid spacing with unresolved
!!    if ((qi1d(k) + qs1d(k) > 1.e-9) .or. w1d(k) > eps .or. dz1d(k) > 500.) then
    if ((qi1d(k) + qs1d(k) > 1.e-9 .and. dz1d(k) > 100.) .or. (w1d(k) > 0.001 .and. qi1d(k) <= r1)) then       
       if (temp(k) < 268.15 .and. temp(k) >= hgfrz) then
          orho = 1._wp / rho(k)
          omega = 0._wp
          !!          w_tmp = min(max(10.**(-0.0869565*(temp(k)-240.)), 0.01),1.)
          !! w_tmp = min(max(10.**(-0.1111111*(temp(k)-245.)), 0.01),1.)
          
          if (dz1d(k) <= 500.) then
             !!             w_tmp = max(10.**(0.01*(dz1d(k)-600.)), 0.01)
! 250             w_tmp = max(0.2 - 0.19 * (10**(-0.008525024*(dz1d(k)-250.))), 0.01)
             w_tmp = max(0.2 - 0.19 * (10**(-0.0056969*(dz1d(k)-100.))), 0.01)             
          else
             w_tmp = 0.2
          endif
          if (qs1d(k) > qi1d(k)) w_tmp = 0._wp
          
          w_tmp = max(w1d(k), w_tmp)
          
          omega = -9.8_wp * w_tmp * rho(k)
          
          !if (w1d(k) > 0.) omega = -9.8_wp * max(w1d(k), 0.01) * rho(k)
          
          !if (temp(k) <= 240.) omega = -9.8_wp * max(w1d(k), 1.) * rho(k)
 
          dqsdti = lsub * qvsi(k) / (rv*temp(k)**2)
          al = 1._wp / (1._wp + dqsdti*lsub*ocp(k))
          bs = al * (1._wp-cf_rhi(k)) * qvsi(k)
          sd = al*(qvsi(k)-qv(k))
          qtot_mean = qv(k) + qi1d(k) !! + qs1d(k)
          qi_mean = al*(qtot_mean-qvsi(k))
          ls_cond = -al * dqsdti * (omega*orho*ocp(k))
!          ssati_total = (qv(k) + qi1d(k) + qs1d(k)) / qvsi(k)
          ssati_total = (qv(k)) / qvsi(k)          
!!!          ssati_total = (qv(k) + qs1d(k)) / qvsi(k)          

!!          if (ssati(k) > (cloud_fraction_rhi-0.99_wp) .and. ssati(k) < 0._wp .and. temp(k) < 263.15 .and. omega < eps) then
          if (ssati_total > (cf_rhi(k)) .and. ssati(k) < 0._wp) then             
             qi_ = qi1d(k) !!!! JULy 09 + tend%prw_ini(k)*orho*dt
             if ((abs(sd) > 1.e-6_wp) .and. qi_ > r1) then
                qifrac_ = qifrac1d(k)
                term1 = ((1._wp-qifrac_)**2*qifrac_**2/qi_) + (qifrac_**2*(1._wp-qifrac_)**2/sd)
                term2 = (1._wp-qifrac_)**2 + qifrac_**2
                gterm = 0.5_wp*term1/term2
                
                ! large-scale forcing
                tend%pra_ini(k) = tend%pra_ini(k) + max(gterm*ls_cond, 0._dp)
!!!                tend%prw_ini(k) = tend%prw_ini(k) + max(min(1., qifrac_ + tend%pra_ini(k)*dt)*ls_cond, 0._dp)
                tend%prw_ini(k) = tend%prw_ini(k) + max(qifrac_*ls_cond, 0._dp)                

             endif
          endif

!!          if (ssati_total > (cloud_fraction_rhi) .and. ssati(k) < 0._wp .and. temp(k) < 240.) then
             
!!          endif
          
          ! new erosion
!          if (qv(k)/qvsi(k) > cloud_fraction_rhi .and. ssati_total-1. < 0._wp) then
!             tend%prw_ier(k) = min(0._dp, 0.3333_wp * 3.e-6_wp * qifrac1d(k) * (1._wp - qifrac1d(k))*(qvsi(k)-qv(k)))
!             tend%pra_ier(k) = min(0._dp, (-gterm * qi_mean) / (qi1d(k) - qifrac1d(k)*qi_mean) * tend%prw_ier(k))
!          endif
...
!    if (tend%prs_ide(k) < 0.) then!
!
!    endif

    ! TURNED BACK ON 13 JUNE 2026
!    if (tend%pri_ide(k)*qifrac1d(k) > r1 .and. qifrac1d(k) >= cf_low) then
!    if (tend%pri_ide(k) > r1 .and. qi1d(k) > r1) then       
! AAJ TEST 4       tend%pra_ini(k) = tend%pra_ini(k) + max((5.57_wp*(1000._wp*tend%pri_ide(k)/qifrac1d(k)*dt)**(0.78_wp) * odt), cf_low)

!!       tend%pra_ini(k) = tend%pra_ini(k) + max((5.57_wp*(1000._wp*tend%pri_ide(k)/qifrac1d(k))**(0.78_wp) * odt), cf_low)

!       tend%pra_ini(k) = tend%pra_ini(k) + ((1.+tend%pri_ide(k)*dt/qi1d(k))**0.5 - 1.) * odt * qifrac1d(k)       
!    endif
...
!      endif
  ...
  subroutine liquid_cloud_fraction(cf_rh, temp, pres, dz1d, l_qc, l_qr, rho, qv, qvs, qc1d, nc1d, qcfrac1d, &
      w1d, lvap, ocp, ssatw, thten_swrad1d, thten_lwrad1d, qvten_bl1d, qcten_bl1d, &
      thten_bl1d, nwfa, qc_bl1d, qcfrac_bl1d, tend, dt, odt)
    ...
    real(wp) :: orho, omega, dqsdt, al, bs, sd, qc_mean, qtot_mean, ls_cond, &
      term1, term2, term3, gterm, eros_term, theta_to_temp, xnc, qcfrac_
    integer :: k, nz
    ...
    do k = 1, nz
      ...
      if (.not. l_qc(k) .and. .not. l_qr(k)) then
...
!!         tend%prw_sgi(k) = qc_bl1d(k) * odt * rho(k)
         ...
         if (ssatw(k) > (cf_rh(k)-0.99_wp) .and. ssatw(k) < 0._wp) then
            ...
            if (temp(k) > 253.15) then            
               tend%pra_sgi(k) = tend%pra_sgi(k) + 0.5_dp/bs*(bs+qc_mean) * odt
               tend%prw_sgi(k) = tend%prw_sgi(k) + tend%pra_sgi(k)*0.5_dp*(bs+qc_mean) * rho(k) ! kg/m3/s
            endif
            ...
            if ((qcfrac1d(k) + tend%pra_sgi(k)*dt) < 0.05) then
              tend%pra_sgi(k) = 0._dp
              tend%prw_sgi(k) = 0._dp
            endif
!            
        ...
        if ((abs(sd) > 1.e-6_wp) .and. qc1d(k) > r1) then
          qcfrac_ = qcfrac1d(k)
...
!          tend%prw_sbl(k) = qcfrac_*al * (qvten_bl1d(k) + qcten_bl1d(k) - dqsdT*(thten_bl1d(k)*theta_to_temp - lvap(k)*ocp(k)*qcten_bl1d(k)))
          ...
          if ((tend%prw_slw(k)+tend%prw_sbl(k))*dt > r1) then
...
!          tend%prw_sge(k) = min(0._dp, ((qc1d(k)-qc_mean*qcfrac_)*eros_term))
          ...
          ! new erosion
          !          if (tend%pra_sgi(k) < eps) then
          if (ssatw(k) > (cf_rh(k)-0.99_wp) .and. ssatw(k) < 0._wp) then
             tend%prw_sge(k) = min(0._dp, 1. * 0.3333_wp * 3.e-4_wp * qcfrac1d(k) * (1._wp - qcfrac1d(k))*(qvs(k)-qv(k)))
             tend%pra_sge(k) = min(0._dp, (-gterm * qc_mean) / (qc1d(k) - qcfrac1d(k)*qc_mean) * tend%prw_sge(k))
          endif

          if (present(nc1d)) then
            if (qc1d(k) > r1) then
              tend%pnc_sgs(k) = tend%pnc_sgs(k) + (tend%prw_sge(k) * nc1d(k)/qc1d(k))
              if ((tend%prw_slw(k) + tend%prw_sbl(k)) < 0.) then
                tend%pnc_sgs(k) = tend%pnc_sgs(k) + ((tend%prw_slw(k)+tend%prw_sbl(k)) * nc1d(k)/qc1d(k))
              endif 
            endif 
          endif 

          if (qcfrac_ + tend%pra_sge(k)*dt < 0.05) then
            tend%pra_sge(k) = -qcfrac1d(k)*odt
            tend%prw_sge(k) = -qc1d(k)*odt
            tend%prw_sbl(k) = 0.
            tend%pra_sbl(k) = 0.
            tend%pra_sgi(k) = 0.
            tend%prw_sgi(k) = 0.
            tend%prw_slw(k) = 0.
            tend%pra_slw(k) = 0.
            if (present(nc1d)) tend%pnc_sgs(k) = -nc1d(k)*odt
          endif

          if ((qc1d(k) + tend%prw_sge(k)*dt) <= r1) then
            tend%pra_sge(k) = -qcfrac1d(k)*odt
            tend%prw_sge(k) = -qc1d(k)*odt
            tend%prw_sbl(k) = 0.
            tend%pra_sbl(k) = 0.
  ...
  subroutine freeze_cloud_melt_ice(temp, rho, ocp, lvap, qi1d, ni1d, qiten, niten, &
    qc1d, nc1d, qcten, ncten, tten, ncsave, dt, odt, qcfrac1d, qifrac1d)
  ...
  subroutine warm_rain(rhof, l_qc, rc, nc, ilamc, mvd_c, l_qr, rr, nr, mvd_r, tend, qcfrac1d, odt)
...
!         if (rc(k)*qcfrac1d(k) > 0.1e-3_wp) then ! .and. qcfrac1d(k) > 0.99) then
        if (rc(k)*qcfrac1d(k) > 0.01e-3_wp) then
          nu_c = get_nuc(nc(k))
          lamc = 1._dp / ilamc(k)       
          ...
          tend%prr_wau(k) = min(real(rc(k)*odt, kind=dp), &
               tend%prr_wau(k))
! JUNE 15 2026          
          tend%pnr_wau(k) = tend%prr_wau(k) / (qcfrac1d(k)*am_r*nu_c*autocon_nr_factor*d0r*d0r*d0r)
!!          tend%pnr_wau(k) = tend%prr_wau(k) / (am_r*nu_c*autocon_nr_factor*d0r*d0r*d0r)
  ...
  subroutine riming(temp, rhof, visco, l_qc, rc, nc, ilamc, mvd_c, &
    ...
    real(wp), intent(in) :: odt    
    type(ty_tend), intent(inout) :: tend
    real(wp), dimension(:), intent(in) :: rhof, visco, temp, rc, nc, rs, rg, ng, qcfrac1d
  ...
  subroutine ice_nucleation(temp, rho, w1d, qv, qvsi, ssati, ssatw, &
    ...
    real(wp), dimension(:), intent(in) :: qv, temp, rho, qvsi, rr, nr, rc, nc, w1d, &
      ssati, ssatw, ni, qifrac1d, qcfrac1d
    ...
    do k = 1, nz
      if (temp(k) < t0) then
        ...
        elseif (rc(k) > r1 .and. temp(k) < hgfrz) then
          tend%pri_wfz(k) = rc(k)*odt !* qcfrac1d(k)
          tend%pni_wfz(k) = nc(k)*odt !* qcfrac1d(k)
        endif

        !>
        !> deposition nucleation from dust is from
        !> [DeMott et al. (2010)](https://doi.org/10.1073/pnas.0910818107)
        if ((ssati(k) >= demott_nuc_ssati - (1.-cloud_fraction_rhi)) .or. (ssatw(k) > (cloud_fraction_rh-1._wp) .and. tempc < -20._wp)) then
!!        if (ssati(k) >= demott_nuc_ssati .or. (ssatw(k) > 0._wp .and. tempc < -20._wp)) then           
          if (present(nifa)) then
  ...
  subroutine ice_processes(rhof, rhof2, rho, w1d, temp, cf_rhi, qv, qvsi, tcond, diffu, &
    vsc2, ssati, l_qi, ri, ni, ilami, l_qs, rs, smoe, smof, smo1, rr, nr, ilamr, &
    mvd_r, l_qg, rg, ng, ilamg, idx, qifrac1d, tend, odt)
    ...
    real(wp), dimension(:), intent(in) :: rhof, rhof2, rho, w1d, ri, ni, rs, rr, nr, &
      temp, qv, qvsi, tcond, diffu, ssati, vsc2, mvd_r, rg, ng, qifrac1d, cf_rhi
    ...
    do k = 1, nz
      ...
      if (temp(k) < t0) then
        if (l_qi(k)) then
          ...
          if (ssati(k) >= cf_rhi(k)-0.99_wp) then
             tend%pri_ide(k) = max(tend%pri_ide(k), 0._dp)
          endif
          
          if (tend%pri_ide(k) < 0._dp) then
            tend%pri_ide(k) = max(real(-ri(k)*odt, kind=dp), tend%pri_ide(k), real(rate_max, kind=dp))
          ...
          tend%pri_ide(k) = tend%pri_ide(k) * qifrac1d(k)
          tend%pni_ide(k) = tend%pni_ide(k) * qifrac1d(k)
          if (qifrac1d(k) > 0.95) then
             tend%prs_ide(k) = tend%prs_ide(k) * qifrac1d(k)
          else
             tend%prs_ide(k) = 0.             
          endif
          ...
          elseif (xdi < 0.1_wp*d0s) then
            ...
            if (qifrac1d(k) > 0.95) then            
              tend%prs_iau(k) = tps_iaus(idx_i,idx_i1)*odt * qifrac1d(k)
              tend%prs_iau(k) = min(real(ri(k)*.99_wp*odt, kind=dp), tend%prs_iau(k))
              tend%pni_iau(k) = tni_iaus(idx_i,idx_i1)*odt * qifrac1d(k)
          ...
          oxmi = 1./xmi
          if (rs(k) >= r_s(1)) then
            tend%prs_sci(k) = t1_qs_qi*rhof(k)*ef_si*ri(k)*smoe(k) * qifrac1d(k)
...
!          if (ssati(k) >= cloud_fraction_rhi-0.99_wp .and. tend%prs_sde(k) < 0.) then
!             tend%prs_iau(k) = 0._dp
!             tend%pni_iau(k) = 0._dp
!             tend%prs_sci(k) = 0._dp
!             tend%pni_sci(k) = 0._dp             
!          endif
          
!             tend%prs_iau(k) = max(tend%prs_iau(k) - tend%prs_iau(k)*tend%prs_sde(k)/rs(k), 0._dp)
!             tend%pni_iau(k) = tend_pni_iau(k) - tend%prs_iau(k)*tend%prs_sde(k)/rs(k)
!          endif

!          if (l_qi(k)) then
!             if (ssati(k) >= cloud_fraction_rhi-0.99_wp) then
!                tend%prs_sde(k) = max(tend%prs_sde(k), 0._dp)
!             endif
!          endif
